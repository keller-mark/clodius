...
def create_fasta_multires(
    ...
    Parameters
    ----------
    fa: pyfastx.Fasta() instance
        The fasta file to aggregate organized by chromosome
    ...
    for chrom, length in zip(chroms, lengths):
        ...
        chrom_subseq = fa[chrom]
        chrom_shape = [5, len(subseq)]

        # print("creating new dataset")
        f["resolutions"][str(curr_resolution)]["values"].create_dataset(
            str(chrom), chrom_shape, compression="gzip"
        )
        ...
        while start < len(chrom_data):
            # see above section
            chrom_data[start : start + chunk_size] = chrom_subseq[
                start : start + chunk_size
            ]
            start += int(min(standard_chunk_size, len(chrom_subseq) - start))
